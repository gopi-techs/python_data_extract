    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            for col_idx, cell_value in enumerate(row_values):
                ...
                if 'TOTAL VALUE' in cell_str.upper():
                    # Look for the value in adjacent columns
                    for next_col in range(col_idx + 1, min(len(row_values), col_idx + 3)):
                        next_value = str(row_values[next_col]).strip()
                        if re.match(r'[\d,]+\.?\d*', next_value) and len(next_value) > 2:
                            if not result['total_value']:  # Take the first one found
                                result['total_value'] = next_value
                                break

                # Alternative pattern: Look for numerical values in rightmost columns
                if col_idx >= len(row_values) - 2:  # Last two columns
                    if re.match(r'[\d,]+\.\d{2}', cell_str) and float(cell_str.replace(',', '')) > 100:
                        # Check if the row contains tax-related keywords
                        ...
                        elif 'TOTAL' in row_text and 'VALUE' in row_text and not result['total_value']:
